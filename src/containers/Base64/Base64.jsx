...
function Base64() {
  ...
  const handleEncode = () => {
    try {
      if (inputTxt !== null || inputTxt !== "") {
        setoutputTxt(window.btoa(inputTxt));
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDecode = () => {
    try {
      if (inputTxt !== null || inputTxt !== "") {
        setoutputTxt(window.atob(inputTxt));
      }
    } catch (error) {
  ...
  return (
    <div className="p-4">
      <div className="text-xl text-center border-b-2">
        Base 64 Encode / Decode{" "}
      </div>
      <div className="w-full  flex flex-col justify-center md:flex-row gap-4 p-2">
